...
function ModifierGroupsEditor({ groups, onChange }: { groups:GroupDraft[]; onChange:(groups:GroupDraft[])=>void }) {
  ...
  return <section className="rounded-2xl border border-gray-100 p-4">
    <div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold text-gray-900">กลุ่มตัวเลือก</h2><p className="text-xs text-gray-400">ตั้งค่าได้แบบ Grab เช่น ตัวเลือกเสริม ระดับความเผ็ด หรือเลือกเนื้อสัตว์</p></div><button type="button" onClick={()=>onChange([...groups,{name:"",required:false,minSelect:"0",maxSelect:"1",options:[{name:"",price:"0"}]}])} className="rounded-xl bg-blue-50 px-3 py-2 text-sm text-blue-600">เพิ่มกลุ่ม</button></div>
