...
export default function OrdersPage() {
  ...
  function lineUnitPrice(item:Cart){return item.price+item.modifiers.filter(modifier=>item.modifierIds.includes(modifier.id)).reduce((sum,modifier)=>sum+modifier.price,0);}
  function add(item:Menu){if(!item.sellable)return;const found=cart.find(i=>i.id===item.id&&!i.modifierIds.length);if(found&&item.maxServings!==null&&found.qty>=item.maxServings){setMessage(`${item.name} เหลือขายได้ ${item.maxServings} ${unit(item)}`);return;}setCart(prev=>found?prev.map(i=>i.lineId===found.lineId?{...i,qty:i.qty+1}:i):[...prev,{...item,lineId:`${item.id}-${Date.now()}-${Math.random().toString(16).slice(2)}`,qty:1,note:"",modifierIds:[]}]);}
  function quantity(lineId:string,delta:number){const item=cart.find(i=>i.lineId===lineId);if(item&&delta>0&&item.maxServings!==null&&item.qty+delta>item.maxServings){setMessage(`${item.name} เหลือขายได้ ${item.maxServings} ${unit(item)}`);return;}setCart(prev=>prev.map(i=>i.lineId===lineId?{...i,qty:i.qty+delta}:i).filter(i=>i.qty>0));}
  function toggleModifier(lineId:string,group:ModifierGroup,modifierId:number){setCart(prev=>prev.map(item=>{if(item.lineId!==lineId)return item;const groupOptionIds=group.options.map(option=>option.id);const selectedInGroup=item.modifierIds.filter(id=>groupOptionIds.includes(id));if(item.modifierIds.includes(modifierId))return {...item,modifierIds:item.modifierIds.filter(id=>id!==modifierId)};const withoutGroup=group.maxSelect===1?item.modifierIds.filter(id=>!groupOptionIds.includes(id)):item.modifierIds;if(group.maxSelect>1&&selectedInGroup.length>=group.maxSelect){setMessage(`เลือก ${group.name} ได้ไม่เกิน ${group.maxSelect} รายการ`);return item;}return {...item,modifierIds:[...withoutGroup,modifierId]};}));}
  ...
  async function submit(){
    ...
    const res=await fetch("/api/orders",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
      ...
      items:cart.map(i=>({menuItemId:i.id,qty:i.qty,note:i.note,modifierIds:i.modifierIds})),discount:activeOrder?0:Number(discount||0),note,
    })});
  ...
  return <div className="grid gap-5 overflow-y-auto p-4 lg:p-6 xl:h-[calc(100dvh-96px)] xl:min-h-0 xl:grid-cols-[minmax(0,1fr)_380px] xl:overflow-hidden">
    <section className="pr-1 xl:min-h-0 xl:overflow-y-auto">
    ...
    <aside className="flex min-h-0 flex-col overflow-hidden rounded-3xl border border-gray-100 bg-white p-5">
      ...
      <div className="mt-3 min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">{cart.map(item=><div key={item.lineId} className="border-b border-gray-100 pb-3"><div className="flex justify-between gap-2"><div className="min-w-0"><p className="font-medium text-sm truncate">{item.name}</p><p className="text-xs text-blue-600">฿{lineUnitPrice(item).toFixed(2)} / {unit(item)}</p></div><div className="flex items-center gap-2"><button onClick={()=>quantity(item.lineId,-1)} className="p-1 bg-gray-100 rounded-full"><Minus size={13}/></button><span className="text-sm">{item.qty}</span><button onClick={()=>quantity(item.lineId,1)} className="p-1 bg-gray-100 rounded-full"><Plus size={13}/></button><button onClick={()=>setCart(c=>c.filter(i=>i.lineId!==item.lineId))} className="text-red-400"><Trash2 size={15}/></button></div></div>{!!item.modifierGroups?.length&&<div className="mt-2 space-y-2">{item.modifierGroups.map(group=><div key={group.id}><p className="mb-1 text-[11px] font-medium text-gray-400">{group.name}{group.required?" *":""}</p><div className="flex flex-wrap gap-1.5">{group.options.map(modifier=><button key={modifier.id} type="button" onClick={()=>toggleModifier(item.lineId,group,modifier.id)} className={`rounded-full border px-2.5 py-1 text-[11px] ${item.modifierIds.includes(modifier.id)?"border-blue-200 bg-blue-50 text-blue-700":"border-gray-100 bg-gray-50 text-gray-500"}`}>{modifier.name}{modifier.price>0?` +฿${modifier.price.toFixed(0)}`:""}</button>)}</div></div>)}</div>}<input value={item.note} onChange={e=>setCart(c=>c.map(i=>i.lineId===item.lineId?{...i,note:e.target.value}:i))} placeholder="หมายเหตุ เช่น ไม่เผ็ด" className="mt-2 w-full bg-gray-50 rounded-lg px-3 py-1.5 text-xs outline-none"/></div>)}{!cart.length&&<p className="text-center text-gray-400 text-sm mt-10">เลือกเมนูเพื่อเริ่มออเดอร์</p>}</div>
